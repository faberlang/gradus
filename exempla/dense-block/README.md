...
2. **GQA attention (causal + RoPE)** — q/k/v projections via `nn.linear`,
   ...
   output projection, and the U1.3 configurable RoPE applied to q/k at
   their positions (U1.4);
3. **residual** — `r1 = x + ctx`;
...
PML3 `transformer_block` pin precedent), compared within the documented
5e-4 absolute tolerance:
