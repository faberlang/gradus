...
(`train.train_step_4x4` → `optimize._sgd_family` list form); printed losses match
...
| Optimizer steps | Tensor SGD: `train.train_step_4x4` → `optimize._sgd_family` list form (the delegating stepper). Parameter/checkpoint: `optimize.step` (U3) fresh-gradient rules, fail-closed; `parameter.mutate` bumps version |
...
update is `train.train_step_4x4` → `optimize._sgd_family` list form. Printed
