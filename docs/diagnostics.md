...
- **Executed tier**: this map documents fail-closed identity on the compiled surface. Focused `faber test` proba execution and broader inference exempla remain on the FMIR lever (CTO8-1); the separate GGUF-A1b synthetic and real-file inspection receipts are recorded in their exemplar READMEs and are not model-execution or inference claims. REF-01-U1.9 (`exempla/dense-prefill-smollm2`) did not reach Gradus typed diagnostics: resume-2 rebuilt packet `faber` at radix `3853d4b8f`, then rust-target emit stopped at the runtime plan (`PKG001:host_provider_selection_invalid`; hosts `processus` manifest does not export `processus:exi`). REF-01-U1.10 (`exempla/dense-prefill-qwen2`) did not reach Gradus typed diagnostics: resume-2 rebuilt packet `faber` at radix `3853d4b8f` (E0432 closed) and stopped at rust-target runtime plan with `PKG001:host_provider_selection_invalid` (`norma:processus` requires `processus:exi`; hosts processus manifest does not export it). Prior stops: E0432 at `b919052f0`; compiler `CODEGEN001` (`dense_qwen2.fab`, definition id 4127). Those are Faber/hosts diagnostics, not a `DenseError` / `DenseQwen2Error` variant.
