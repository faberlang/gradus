...
**Status**: active — all 5 units landed (23d951b, 601d2f5, c67f552, f51f299+d6138d5, 7646ed7); goal-closeout audit is the remaining gate to done
**Created**: 2026-08-21
**Campaign:** `—` (standalone)
...
| # | Probe | Observed |
| --- | --- | --- |
| E1 | The claim's origin | `README.md` (added `cd3883d`, 2026-08-04): a stale claim that library-to-library calls were unavailable, so `train_step_*` carried the update math inline rather than calling the optimizer. |
...
| E5 | Delegation landed | `src/train.fab` `train_step_2x2/_4x4` delegate to `optimize.sgd_step_*`; the BERT pair delegates through `optimize._sgd_family`. The inline update duplicates were removed from `train.fab`; `optimize` owns the SGD math. |
| E6 | Stale claims tree-wide | ~20 live files carried now-stale FMIR/library-import-gap headers: `src/{train,optimize,loss,metrics,generation,decode,gradus}.fab`, `src/{train,optimize,gradient,nn,metrics,loss,transformer,sampling,math}.proba`, README. The training-loop header was corrected in U4 and now records the executed 100-step convergence proof. |
...
1. **Optimizer surface shape** — settled: tensor-level
   `sgd_step_2x2`/`sgd_step_4x4` under `gradus:optimize` (retired-ledger
   shapes, per-tensor in/out) so delegation is a drop-in. The state-contract
   alternative (`Parameter`/`Gradient`/`SgdState` via `optimize.step`) was
   rejected because it changes the admitted caller surface.
2. **`_sgd_family` placement** — settled: the shape-generic helper lives in
   `gradus:optimize` and both the fixed-shape steppers and the BERT pair
   delegate through it. The per-shape-only alternative was rejected to avoid
   another copy of the update math.
