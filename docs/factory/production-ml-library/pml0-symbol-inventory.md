...
modules) and the tracked total **597** (the live all-module count — every
...
GGUF-A3 inventory baseline (29 modules / 597). This
...
- Per-module counts for **all 29 live modules** match the live tree exactly
  ...
  tokenizer 23.
- The tracked total == the live all-module total == **597**; every module is
...
./scripta/inventory-public-symbols            # exit 0; per-module counts + total 597 + coverage gate
...
grep -c 'functio ' src/*.fab src/model/*.fab | awk -F: '{s+=$2} END {print s}'   # 597 (live all-module)
git diff --check
```

Outcome: `./scripta/inventory-public-symbols` exits 0 (per-module baseline
and tracked total 597 hold; every public symbol is documented in
`docs/api-reference.md`); a fresh run diffs clean against the captured output
above; the live all-module total == 597 matches live grep; `git diff --check`
clean.
