...
| `tests/admission_conformance.fab` | 7 SEM006 (all `tokenizator.*` — covered by the tokenizer unit) |
...
| Module | First site |
| --- | --- |
| math | `src/attention.proba:72:15` (`math.casta`) |
| dtype | `src/dtype.proba:49:33` (`dtype.deserializa`) |
| shape | `src/shape.proba:42:21` (`shape.gradus`) |
| tensor | `src/tensor.proba` (genus/surface errors — module-private error type) |
| cache | `src/cache.proba:79:29` (`cache.KVCache`) |
| decode | `src/decode.proba:140:35` (`decode.praefundere`) |
| generation | `src/generation.proba:62:15` (`generation.causa`) |
| gradient | `src/gradient.proba:76:32` (`gradient.Gradientes`) |
| loss | `src/loss.proba:77:25` (`loss.mse`) |
| metrics | `src/metrics.proba:80:25` (`metrics.accuratezza`) |
...
| train | `src/train.proba:80:21` (`train.Modus`) |
| parameter | `src/parameter.proba:40:26` (`parameter.Identitas`) |
| optimize | `src/optimize.proba` (`optimize.*` sites) |
| serialize | `src/serialize.proba:117:25` (`serialize.Tensum`) |
| tokenizer | `src/tokenizer.proba:54:27` (`tokenizer.IdentitasTokenizator`) |
| sampling | `src/sampling.proba:124:15` (`sampling.distributio`) |
| gradus | `src/gradus.proba:97:15` (`gradus.causa`) |
...
| --- | --- |
| `src/math.fab` | MathError@78, structa@278, sub@316, div@351, neg@368, abs@379, signum@396, summa@411, media@447, casta@545, concatenatio@568, segmentum@634 |
| `src/dtype.fab` | DTypeError@124, amplitudo@165, serializa@178, deserializa@182, promovet@203, angusta@239 |
| `src/shape.fab` | FormaError@85, gradus@118, reformanda@219, expansio@277 |
| `src/tensor.fab` | TensorError@119 |
| `src/cache.fab` | CacheError@62, causa@73, KVCache@162, cache_aequus@243, cache_vacua@265, appende@313, redintegra@367, IdentitasCache@393, identitas_cache_aequus@416, identitas_cache@431, serializa_identitas@458, deserializa_identitas@466 |
| `src/decode.fab` | DecodeError@103, praefundere@442, progredere@503, cancelatum_cancellata@542, replica@574 |
| `src/generation.fab` | GeneratioError@88, causa@98, generatio_aequus@176, generatio_defecta@256, imperia_subsidia@266, imperium_admissum@272, serializa_generatio@318, deserializa_generatio@339, cursor_redintegra@517 |
| `src/gradient.fab` | GradienteError@71, causa@76, Gradientes@145, structa_gradientes@163 |
| `src/loss.fab` | LossError@93, causa@105, mse@220, cross_entropy@261 |
| `src/metrics.fab` | MetricError@54, causa@64, accuratezza@85, metrica_aequus@188 |
...
| `src/train.fab` | TrainError@280, Modus@423, modus_nomen@429, est_disciplina@445, est_aestimatio@450, modus@457, dropout_pars@472, Fructus@563, proximus@580, Excutio@630, excutio@645, serializa_semen@678, deserializa_semen@682 |
| `src/parameter.fab` | statio_nomen@88, ParametrumError@100, Identitas@134, identitas_aequus@170, est_trainabilis@253, structa_gelida@313, Registrum@355, registrum_vacuum@413, adscisco@419, serializa@465, deserializa@473 |
| `src/optimize.fab` | OptimizeError@115, causa@132, statum_aequus@231, sgd_aequus@297, serializa_statum@405, deserializa_statum@410 |
| `src/serialize.fab` | SerializeError@117, causa@126, Tensum@144, ParametrumWire@166, serializa_dtype@460, serializa_shape@472, serializa_tensor@506, serializa_parametrum@543, deserializa_dtype@634, deserializa_shape@647, deserializa_tensor@677, deserializa_parametrum@719 |
| `src/tokenizer.fab` | TokenizerError@207, causa@219, IdentitasTokenizator@240, proba_aequa@352, proba_ida@368, verifica_proba@402, pinnata_proba@412, structa@437, verifica@495, clavis_tokenizatoris@535, serializa_identitas@550, deserializa_identitas@557 |
| `src/sampling.fab` | distributio@200 |
| `src/gradus.fab` | GradusError@89, causa@100 |
| `src/model/gguf.fab` | GgufError@149, causa@162, admit@331 |
...
`grep -n '^\(functio\|genus\|structa\|discretio\) <name>' <file>` — the first
top-level match with that name is the target (member functions are indented and are
**not** targets; note `metrics.accuratezza` has an indented member overload — annotate
...
| Field | Value |
| --- | --- |
| outcome | math family public-import surface complete: MathError + structa/sub/div/neg/abs/signum/summa/media/casta/concatenatio/segmentum `@ publica`; all SEM006 referencing `math.` cleared tree-wide (95 visible sites at baseline) |
| exact write scope | `src/math.fab` — `@ publica` immediately above each of the 12 §4 anchors (MathError@78, structa@278, sub@316, div@351, neg@368, abs@379, signum@396, summa@411, media@447, casta@545, concatenatio@568, segmentum@634) |
| first failing oracle | `src/attention.proba:72:15` SEM006 (`math.casta`) |
...
| Field | Value |
| --- | --- |
| outcome | dtype family public-import surface complete: DTypeError + amplitudo/serializa/deserializa/promovet/angusta `@ publica`; all SEM006 referencing `dtype.` cleared (35 visible sites) |
| exact write scope | `src/dtype.fab` — `@ publica` above DTypeError@124, amplitudo@165, serializa@178, deserializa@182, promovet@203, angusta@239 |
| first failing oracle | `src/dtype.proba:49:33` SEM006 (`dtype.deserializa`) |
...
| Field | Value |
| --- | --- |
| outcome | shape family public-import surface complete: FormaError + gradus/reformanda/expansio `@ publica`; all SEM006 referencing `forma.` cleared (21 visible sites) |
| exact write scope | `src/shape.fab` — `@ publica` above FormaError@85, gradus@118, reformanda@219, expansio@277 |
| first failing oracle | `src/shape.proba:42:21` SEM006 (`shape.gradus`) |
| closeout command | common closeout with `<X>` = shape |
| expected observed result | zero SEM006 referencing `forma.` in the sweep; `faber test .` SEM006 drops by ≥21 |
| est_basis | pilot; 4 one-line insertions + narrow checks |
| stop condition | all 4 anchors annotated; zero `forma.` SEM006 in the sweep; `git diff --check` silent |
| depends_on | base `698d12f` |
...
| Field | Value |
| --- | --- |
| outcome | cache family public-import surface complete: CacheError, KVCache, IdentitasCache + causa/cache_aequus/cache_vacua/appende/redintegra/identitas_cache_aequus/identitas_cache/serializa_identitas/deserializa_identitas `@ publica`; all SEM006 referencing `cache.` cleared (33 visible + masked sites) |
| exact write scope | `src/cache.fab` — `@ publica` above the 12 §4 anchors (CacheError@62 … deserializa_identitas@466) |
...
| Field | Value |
| --- | --- |
| outcome | decode family public-import surface complete: DecodeError + praefundere/progredere/cancelatum_cancellata/replica `@ publica`; all SEM006 referencing `decode.` cleared (16 visible sites) |
| exact write scope | `src/decode.fab` — `@ publica` above DecodeError@103, praefundere@442, progredere@503, cancelatum_cancellata@542, replica@574 |
| first failing oracle | `src/decode.proba:140:35` SEM006 (`decode.praefundere`) |
...
| Field | Value |
| --- | --- |
| outcome | generation family public-import surface complete: GeneratioError + causa/generatio_aequus/generatio_defecta/imperia_subsidia/imperium_admissum/serializa_generatio/deserializa_generatio/cursor_redintegra `@ publica`; all SEM006 referencing `generatio.` cleared (32 visible sites) |
| exact write scope | `src/generation.fab` — `@ publica` above the 9 §4 anchors (GeneratioError@88 … cursor_redintegra@517) |
| first failing oracle | `src/generation.proba:62:15` SEM006 (`generation.causa`) |
| closeout command | common closeout with `<X>` = generation |
| expected observed result | zero SEM006 referencing `generatio.` in the sweep; `faber test .` SEM006 drops by ≥32 |
| est_basis | pilot; 9 one-line insertions + narrow checks |
| stop condition | all 9 anchors annotated; zero `generatio.` SEM006 in the sweep; `git diff --check` silent |
| depends_on | base `698d12f` |
...
| Field | Value |
| --- | --- |
| outcome | gradient family public-import surface complete: GradienteError, Gradientes + causa/structa_gradientes `@ publica`; all SEM006 referencing `gradient.` cleared (5 visible + masked sites) |
| exact write scope | `src/gradient.fab` — `@ publica` above GradienteError@71, causa@76, Gradientes@145, structa_gradientes@163 |
| first failing oracle | `src/gradient.proba:76:32` SEM006 (`gradient.Gradientes`) |
...
| Field | Value |
| --- | --- |
| outcome | loss family public-import surface complete: LossError + causa/mse/cross_entropy `@ publica`; all SEM006 referencing `loss.` cleared (19 visible sites) |
| exact write scope | `src/loss.fab` — `@ publica` above LossError@93, causa@105, mse@220, cross_entropy@261 |
...
| Field | Value |
| --- | --- |
| outcome | metrics family public-import surface complete: MetricError + causa/accuratezza (top-level overload)/metrica_aequus `@ publica`; all SEM006 referencing `metrica.` cleared (11 visible sites) |
| exact write scope | `src/metrics.fab` — `@ publica` above MetricError@54, causa@64, accuratezza@85, metrica_aequus@188 (the indented member `accuratezza()` inside Metricum is NOT a target) |
| first failing oracle | `src/metrics.proba:80:25` SEM006 (`metrics.accuratezza`) |
...
| Field | Value |
| --- | --- |
| outcome | train family public-import surface complete: TrainError, Modus, Fructus, Excutio + modus_nomen/est_disciplina/est_aestimatio/modus/dropout_pars/proximus/excutio/serializa_semen/deserializa_semen `@ publica`; all SEM006 referencing `train.` cleared (30 visible + masked sites) |
| exact write scope | `src/train.fab` — `@ publica` above the 13 §4 anchors (TrainError@280 … deserializa_semen@682) |
| first failing oracle | `src/train.proba:80:21` SEM006 (`train.Modus`) |
...
| Field | Value |
| --- | --- |
| outcome | parameter family public-import surface complete: ParametrumError, Identitas, Registrum + statio_nomen/identitas_aequus/est_trainabilis/structa_gelida/registrum_vacuum/adscisco/serializa/deserializa `@ publica`; all SEM006 referencing `parametrum.` cleared (19 visible + masked sites) |
| exact write scope | `src/parameter.fab` — `@ publica` above the 11 §4 anchors (statio_nomen@88 … deserializa@473) |
| first failing oracle | `src/parameter.proba:40:26` SEM006 (`parameter.Identitas`) |
...
| Field | Value |
| --- | --- |
| outcome | optimize family public-import surface complete: OptimizeError + causa/statum_aequus/sgd_aequus/serializa_statum/deserializa_statum `@ publica`; all SEM006 referencing `optimize.` cleared (masked sites in optimize.proba/train.proba) |
| exact write scope | `src/optimize.fab` — `@ publica` above OptimizeError@115, causa@132, statum_aequus@231, sgd_aequus@297, serializa_statum@405, deserializa_statum@410 |
...
| Field | Value |
| --- | --- |
| outcome | serialize family public-import surface complete: SerializeError, Tensum, ParametrumWire + causa/serializa_dtype/serializa_shape/serializa_tensor/serializa_parametrum/deserializa_dtype/deserializa_shape/deserializa_tensor/deserializa_parametrum `@ publica`; all SEM006 referencing `serialize.` cleared (4 visible + masked sites) |
| exact write scope | `src/serialize.fab` — `@ publica` above the 12 §4 anchors (SerializeError@117 … deserializa_parametrum@719) |
| first failing oracle | `src/serialize.proba:117:25` SEM006 (`serialize.Tensum`) |
...
| Field | Value |
| --- | --- |
| outcome | tokenizer family public-import surface complete: TokenizerError, IdentitasTokenizator + causa/proba_aequa/proba_ida/verifica_proba/pinnata_proba/structa/verifica/clavis_tokenizatoris/serializa_identitas/deserializa_identitas `@ publica`; all SEM006 referencing `tokenizator.` cleared (15 visible + masked sites; also clears the 7 `tests/admission_conformance.fab` sites) |
| exact write scope | `src/tokenizer.fab` — `@ publica` above the 12 §4 anchors (TokenizerError@207 … deserializa_identitas@557) |
| first failing oracle | `src/tokenizer.proba:54:27` SEM006 (`tokenizer.IdentitasTokenizator`) |
| closeout command | common closeout with `<X>` = tokenizer (sweep includes `tests/admission_conformance.fab`) |
| expected observed result | zero SEM006 referencing `tokenizator.` in the sweep (including admission_conformance.fab); `faber test .` SEM006 drops by ≥15 |
...
| stop condition | all 12 anchors annotated; zero `tokenizator.` SEM006 in the sweep; `git diff --check` silent |
| depends_on | base `698d12f` |
...
| Field | Value |
| --- | --- |
| outcome | sampling family public-import surface complete: distributio `@ publica`; all SEM006 referencing `sampling.` cleared (1 visible site) |
| exact write scope | `src/sampling.fab` — `@ publica` above distributio@200 only |
| first failing oracle | `src/sampling.proba:124:15` SEM006 (`sampling.distributio`) |
| closeout command | common closeout with `<X>` = sampling |
| expected observed result | zero SEM006 referencing `sampling.` in the sweep |
| est_basis | pilot; 1 one-line insertion + narrow checks (small unit — batchable at dispatch) |
| stop condition | distributio annotated; zero `sampling.` SEM006 in the sweep; `git diff --check` silent |
| depends_on | base `698d12f` |
...
| Field | Value |
| --- | --- |
| outcome | gradus facade public surface complete: GradusError + causa `@ publica`; all SEM006 referencing `gradus.` cleared (1 visible site) |
| exact write scope | `src/gradus.fab` — `@ publica` above GradusError@89, causa@100 |
| first failing oracle | `src/gradus.proba:97:15` SEM006 (`gradus.causa`) |
...
| Field | Value |
| --- | --- |
| outcome | gguf admission family public-import surface complete: GgufError + causa/admit `@ publica`; all SEM006 referencing `gguf.` cleared (7 visible sites) |
| exact write scope | `src/model/gguf.fab` — `@ publica` above GgufError@149, causa@162, admit@331 |
...
| Field | Value |
| --- | --- |
| outcome | artifact family public surface complete: ArtifactError `@ publica` (identitas/causa already publica via `5445111`); artifact.proba's error surface SEM006-free |
