...
| `id` | `GGUF-A7-U3` |
| `classification` | needs-rebaseline |
| `outcome` | Execute the existing native Q4_K_M packed route for SmolLM2-360M on **RunPod CUDA (1× A100-SXM4-80GB, lane dc-a100, image runpod/pytorch:1.0.2-cu1281-torch280-ubuntu2404)** through the current NVVM emitter and the extracted `host-cuda` route. Prove the same block, model, memory, and resident-reuse facts as U2. [Environment amendment 2026-08-26: operator directive "pharos is turned off. use runpod." — replaces pharos; prior honest NOT-ATTEMPTED receipt 60ec5d28e stands as the pharos-era record; spend authorized 8b4e66d5 + in-session; provisioning flow per ruling c9faac9b; receipts under ~/work/ianzepp/trials/runpod-gpu-verification/a7-u3-*/ with zero-orphan teardown] |
