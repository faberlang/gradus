...
| U4 partial note (CTO Q2) | Runtime identity of bare forward vs the generated backward companion is **NOT claimed**: proba execution is env-blocked tree-wide today, so no executed-identity or numerical-bound-at-runtime evidence exists. Deferred to a runtime-evidence gate — recorded as residual #2 below | **PARTIAL — recorded, admitted** |
...
| 2 | Runtime-evidence gate for U4: execute bare forward vs the generated backward companion and record a numerical identity bound (CTO Q2) once proba execution is unblocked (env-blocked tree-wide today); then the training row's identity claim can be upgraded from structural to executed | auditor / faber test path (runtime-evidence gate) | deferred — env-blocked |
