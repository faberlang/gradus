...
**Status**: re-lowered 2026-08-13 by planner-36 (task `d5367a87` / need `0418ad42`); **amended 2026-08-14 by planner-36 (task `954f8d4a`)** — closed the audit `2786c77d` P2 (report `d454793a`): every M3-U1…U8 micro-unit now carries **all eight campaign rule-2 fields** (executed outcome, exact write scope, predecessor receipt, first failing oracle, closeout command, expected observed result, work-token estimate basis, stop condition), per-unit; the aggregate fields (§7 gates, §8 oracle) remain lane-owned and are *not* duplicated onto children. — **READY at the spec level; dispatch-gated on MODEL-01 (GGUF-M1) landing and the GGUF-A1c/A2/A3/A5 predecessor set** (see §Entry Gate). Planning artifact only: no product code is written by this lowering.
...
Units are listed in dependency order. Fields follow the standing Hand-unit schema; every unit carries **all eight campaign rule-2 fields** (executed outcome, exact write scope, predecessor receipt, first failing oracle, closeout command, expected observed result, work-token estimate basis, stop condition) — restored per-unit by the audit `2786c77d` P2 fix — plus `est_work_tokens` 3k–8k, `tool_latency`, `sanity`, `non_goals`, `risk`, `integrable`, and **1–3 primary files**. None carries a lane gate (§Aggregate Gate). Final module file names follow MODEL-01's `qwen35moe` naming at dispatch; the names below are the recommended shape.
...
- **first failing oracle**: the admitted MODEL-01 tensor inventory vs the derived per-block type — the first contradictory tensor (named block + tensor name + expected vs actual) is the divergence unit, never a whole-inventory claim; gate row is the exact Qwen3.6 artifact, heretic/ornith are compile-level stress rows (no executed claim).
- **depends_on**: MODEL-01 (GGUF-M1) admission + tensor map.
- **sanity**: one narrow proba run over the schedule cases.
- **closeout command** (from the Hand packet; no lane gates per §Aggregate Gate):
  ```bash
  ...
  env FABER_LIBRARY_HOME=/Users/ianzepp/work/faberlang/worktrees/<hand> \
    ...
    <faber> test src/model/qwen35moe_schedule.proba
  git diff --check -- src/model/qwen35moe_schedule.fab src/model/qwen35moe_schedule.proba \
    docs/module-map.md docs/api-reference.md docs/diagnostics.md
  ```
  (if the FMIR/library-import seam still blocks executed proba runs, the proba is compile-level structural evidence and the executed schedule claim is proven at the M3 aggregate per §FMIR Gate).
- **expected observed result**: the derived schedule is SSM on `(i+1)%4 != 0` of blocks 0..39, full attention on `{3,7,…,39}`, block 40 = MTP for the exact artifact; validation is silent for the gate row; a mutated inventory prints the typed diagnostic naming block + first tensor; the unit proba is green; `git diff --check` silent.
...
- **stop condition**: pause and route when — the admitted config/tensor inventory cannot be consumed as typed values (defect in MODEL-01's admission → route to MODEL-01); a schedule fact required by §F1 cannot be represented in the typed surface; a contradiction cannot be expressed as a typed diagnostic.
- **integrable**: yes.
- **est_work_tokens**: 3k–5k.
- **est_basis**: the narrowest slice of the original 36k–72k (schedule derivation only — a pure function over the admitted config); nearest precedent is the A1a/gguf-inspect parser-validated-module shape, not the math or probe units; per-unit ranges sum to 37k–55k across U1–U8, inside the original single-unit band (no scope growth).
- **tool_latency**: low — one proba run over in-memory schedule cases; no artifact load, no cargo.
...
- **first failing oracle**: the MODEL-01 dimension facts (§F2) and the typed state contract — the first failing case is the first probe whose fresh/append/reset/equality/identity behavior diverges from the pinned contract; the F8 identity round-trip is the second independent check (first mismatched key field), not a text-level comparison.
...
- **closeout command** (from the Hand packet; no lane gates per §Aggregate Gate):
  ...
  env FABER_LIBRARY_HOME=/Users/ianzepp/work/faberlang/worktrees/<hand> \
    FABER_BIN=/Users/ianzepp/work/faberlang/worktrees/<hand>/radix/target/debug/faber \
    <faber> test src/model/qwen35moe_state.proba
  git diff --check -- src/model/qwen35moe_state.fab src/model/qwen35moe_state.proba \
    docs/module-map.md docs/api-reference.md docs/diagnostics.md
  ```
  (FMIR seam caveat as U1; the executed state semantics are re-proven at U6/U8).
- **expected observed result**: fresh construction yields empty conv / zero recurrent / empty KV; append/update mutate per the typed contract; reset returns the fresh-session value; equality is strict; identity round-trips with every F8 field bound; the unit proba is green; `git diff --check` silent.
- **non_goals**: the update/output math (U3), attention math (U4), schedule derivation (U1).
- **risk**: low.
- **stop condition**: pause and route when — a state/conv/KV dimension or identity fact required by §F2/F8 cannot be represented as a typed value (defect in MODEL-01's admitted facts → route to MODEL-01); the identity key cannot be bound without a device handle, path, or reader (forbidden by constraint — stop, route to the device-neutrality owner).
- **integrable**: yes.
- **est_work_tokens**: 3k–5k.
- **est_basis**: typed-value construction + identity wire — precedent is the PML5-U2 `cache.fab` identity-key surface (landed), not the math units; a small fraction of the original 36k–72k (state values slice).
- **tool_latency**: low — one proba run over in-memory state cases; no artifact load, no cargo.
...
- **first failing oracle**: the committed independent reference (gi2-dequant-reference precedent) on the pinned hidden-state probes — the first divergent element of the update or output under the settled band is the divergence unit (layer/position/element granularity, never an aggregate-similarity claim); the algebraic form is settled by this first-failing case, never by independent derivation.
...
- **closeout command** (from the Hand packet; no lane gates per §Aggregate Gate):
  ...
  env FABER_LIBRARY_HOME=/Users/ianzepp/work/faberlang/worktrees/<hand> \
    FABER_BIN=/Users/ianzepp/work/faberlang/worktrees/<hand>/radix/target/debug/faber \
    <faber> test src/model/qwen35moe_deltanet.proba
  git diff --check -- src/model/qwen35moe_deltanet.fab src/model/qwen35moe_deltanet.proba \
    docs/module-map.md docs/api-reference.md docs/diagnostics.md
  ```
  (`src/nn.fab`/`src/nn.proba` only if this unit added `silu`/`rmsnorm`; FMIR seam caveat as U1).
- **expected observed result**: on the pinned probes the update and output match the committed reference under the settled band; non-finite/shape-mismatch probes fail typed; the unit proba is green; `git diff --check` silent.
...
- **stop condition**: pause and route when — the first-failing oracle cannot settle the exact algebraic form (band methodology or probe plumbing → route to the GI0/evidence owner); a required tensor slice cannot be dequantized from the artifact (→ route to the codec owner); the FMIR stepper/library-import seam cannot execute the value proof (→ route to hand-2/Faber per §FMIR Gate).
- **integrable**: yes.
- **est_work_tokens**: 5k–8k.
- **est_basis**: one math family with oracle settling — the delta part of the original 36k–72k (which priced the full S2–S4 math batch); precedent is the gi2-dequant-reference goldens workflow; mid-band of the per-unit ranges.
- **tool_latency**: low–medium — the pinned-probe goldens are committed once at red-proof; the proba run itself is in-memory.
...
- **first failing oracle**: the committed independent reference on the pinned attention probes (same gi2-dequant-reference methodology as U3) — the first divergent KV value or gated-output element under the settled band; the GGUF-A5 prefill/one-token-decode boundary is the second oracle (first divergent position at the declared boundary).
...
- **closeout command** (from the Hand packet; no lane gates per §Aggregate Gate):
  ...
  env FABER_LIBRARY_HOME=/Users/ianzepp/work/faberlang/worktrees/<hand> \
    FABER_BIN=/Users/ianzepp/work/faberlang/worktrees/<hand>/radix/target/debug/faber \
    <faber> test src/model/qwen35moe_attention.proba
  git diff --check -- src/model/qwen35moe_attention.fab src/model/qwen35moe_attention.proba \
    docs/module-map.md docs/api-reference.md docs/diagnostics.md
  ```
  (FMIR seam caveat as U1).
- **expected observed result**: on the pinned probes, per-position KV append/update/reset holds; the gated attention-subblock output matches the committed reference under the settled band; KV state remains a typed logical value; the unit proba is green; `git diff --check` silent.
- **non_goals**: SSM update math (U3), schedule (U1), trunk composition (U6).
- **risk**: medium.
- **stop condition**: pause and route when — the first-failing oracle cannot settle the gated-attention form (→ route to the GI0/evidence owner); KV/prefill-decode disagreement cannot be attributed to a named position at the declared boundary (→ route to GGUF-A5); the FMIR seam cannot execute the value proof (→ hand-2/Faber per §FMIR Gate).
- **integrable**: yes.
- **est_work_tokens**: 5k–8k.
- **est_basis**: one attention family + KV position state — the attention part of the original S2–S4 math batch; same goldens workflow precedent as U3; mid-band of the per-unit ranges.
- **tool_latency**: low–medium — the attention goldens are committed once at red-proof; the proba run is in-memory.
...
- **first failing oracle**: the committed independent reference for the multi-section rotary variant on the pinned position vectors — the first divergent element under the settled band; the dense `gradus:attention` RoPE (`freq_base` 100000) is a negative oracle only (proves no cross-contamination by staying untouched).
...
- **closeout command** (from the Hand packet; no lane gates per §Aggregate Gate):
  ...
  env FABER_LIBRARY_HOME=/Users/ianzepp/work/faberlang/worktrees/<hand> \
    FABER_BIN=/Users/ianzepp/work/faberlang/worktrees/<hand>/radix/target/debug/faber \
    <faber> test src/model/qwen35moe_rope.proba
  git diff --check -- src/model/qwen35moe_rope.fab src/model/qwen35moe_rope.proba \
    docs/module-map.md docs/api-reference.md docs/diagnostics.md
  ```
  (FMIR seam caveat as U1).
- **expected observed result**: RoPE of the pinned position vectors matches the committed reference under the settled band; `src/attention.fab` is absent from the unit diff (dense RoPE untouched); the unit proba is green; `git diff --check` silent.
- **non_goals**: attention/KV math (U4), any edit to the dense RoPE surface.
- **risk**: medium.
- **stop condition**: pause and route when — a multi-section RoPE fact (§F4) cannot be represented (→ route to MODEL-01); the first-failing oracle cannot settle the rotary form (→ route to the GI0/evidence owner); satisfying the unit would require editing the dense `gradus:attention` RoPE (forbidden — stop, route to the dense-row owner).
- **integrable**: yes.
- **est_work_tokens**: 3k–5k.
- **est_basis**: a standalone rotary variant module — the position-handling slice of the original 36k–72k; precedent is the existing dense RoPE module's shape; low end of the per-unit ranges.
- **tool_latency**: low — one proba run over pinned position vectors; no artifact load.
...
- **first failing oracle**: the trunk's own self-consistency contract (§F5/F6) — per-layer prefill vs one-token decode equivalence, first divergent layer/position; reset/replay determinism, first divergent layer/state value. (The pinned llama.cpp probe is the U8 oracle, not this unit's; this unit settles the Gradus-internal equivalence first.)
...
- **closeout command** (from the Hand packet; no lane gates per §Aggregate Gate):
  ...
  env FABER_LIBRARY_HOME=/Users/ianzepp/work/faberlang/worktrees/<hand> \
    FABER_BIN=/Users/ianzepp/work/faberlang/worktrees/<hand>/radix/target/debug/faber \
    <faber> test src/model/qwen35moe_trunk.proba
  git diff --check -- src/model/qwen35moe_trunk.fab src/model/qwen35moe_trunk.proba \
    docs/module-map.md docs/api-reference.md docs/diagnostics.md
  ```
  (FMIR seam caveat as U1 — the historical stepper limitation is re-verified here per §FMIR Gate).
- **expected observed result**: over the pinned token sequence, per-layer prefill state and attention-subblock output equal incremental one-token decode per layer and position; fresh-session reset and same-prompt replay reproduce identical per-layer state and outputs; block 40's tensors are admitted but never executed in the trunk pass; the unit proba is green; `git diff --check` silent.
...
- **stop condition**: pause and route when — the FMIR stepper/library-import seam cannot execute the equivalence/replay value proofs (→ route to hand-2/Faber per §FMIR Gate — MODEL-03 does not work around it); a prefill/decode or replay divergence cannot be attributed to a named layer/position/state value; block 40 cannot be admitted-without-execution in the typed surface.
- **integrable**: yes.
- **est_work_tokens**: 6k–8k.
- **est_basis**: the composition slice of the original 36k–72k (S5 equivalence/reset/replay) — the largest non-oracle slice, priced above the math units because it composes U1–U5 over 40 layers; high end of the per-unit ranges.
- **tool_latency**: medium — the equivalence/replay proba exercises the full trunk over the pinned sequence; still in-memory, no artifact load.
...
- **first failing oracle**: the pinned llama.cpp build itself (10150 / `dee2a846b`) — the probe is the oracle; the first failing item is the first named tensor that cannot be dumped or whose identity/order diverges from the §Oracle list, and the recorded binary SHA-256 must match the pinned lineage (mismatch stops the harness before any comparison claim).
...
- **closeout command** (evidence lane — the radix evidence packet; no gradus write):
  ```bash
  cd /Users/ianzepp/work/faberlang/worktrees/<hand>/radix/docs/factory/gpu-inference-gguf/evidence
  shasum -a 256 <pinned-build-binary>            # record; must match the pinned lineage
  <probe-build>/dump-per-layer <Qwen3.6-artifact> <prompt> -o <probe-dump-dir>
  python3 compare-per-layer.py <probe-dump-dir> --gate <band>
  git diff --check -- docs/factory/gpu-inference-gguf/evidence
  ```
  (the Qwen3.6 EOG/numeric re-derivation via the gi0-6 method is a dispatch-time precondition that freezes `<band>`).
- **expected observed result**: the probe dumps the named §Oracle tensors for the pinned prompt at the pinned build; the recorded binary SHA-256 matches the pinned lineage; `compare-per-layer.py` reports per-layer/per-position agreement with `DIVERGENCE=none` or the named first layer/position + failing thresholds; the EOG/numeric re-derivation is recorded; `git diff --check` silent.
...
- **stop condition**: pause and route when — the pinned build cannot dump the named tensors (the `cb()`-hook fallback needs the delivery audit's acceptance per §Open Items before it replaces the probe); the binary SHA-256 does not match the pinned lineage; the EOG/numeric re-derivation cannot freeze a band (→ route to the GI0 methodology owner).
- **integrable**: yes (evidence-only).
- **est_work_tokens**: 6k–8k.
- **est_basis**: the probe-harness slice of the original 36k–72k (which priced the harness and its friction in the upper half) — pinned-build instrumentation, dump plumbing, and comparison script; no gradus code; high end of the per-unit ranges.
- **tool_latency**: medium-high — the pinned probe build once, then per-layer dump loads of the 22.6 GB artifact (Metal) at minutes each.
...
- **first failing oracle**: the pinned probe dump (M3-U7) vs the Gradus trunk per-layer state/output — the first divergent layer/position under the first-divergence rule (never hidden by text-level similarity; finite-value gate); reset/replay determinism is the second oracle (first divergent state value); the EOG/numeric re-derivation frozen at dispatch is a precondition, not a softener.
...
- **closeout command** (from the Hand packet; the aggregate acceptance is merge + test lane-owned per §Aggregate Gate):
  ```bash
  cd /Users/ianzepp/work/faberlang/worktrees/<hand>/gradus
  env FABER_LIBRARY_HOME=/Users/ianzepp/work/faberlang/worktrees/<hand> \
    FABER_BIN=/Users/ianzepp/work/faberlang/worktrees/<hand>/radix/target/debug/faber \
    <faber> run --target fmir exempla/qwen35moe-layer-probe -- <probe-dump-dir> --gate <band>
  python3 <evidence>/compare-per-layer.py <probe-dump-dir> --gate <band>
  git diff --check -- exempla/qwen35moe-layer-probe scripta/check-compile \
    docs/regression-corpus.md docs/factory/production-ml-library/pml0-symbol-inventory.md \
    docs/factory/production-ml-library/pml0-support-matrix.md
  ```
  (the FMIR stepper/library-import seam is re-checked at this unit's red-proof per §FMIR Gate).
- **expected observed result**: the per-layer comparison reports agreement at every layer/position up to the first-divergence boundary with `DIVERGENCE=none` (or an honest named first layer/position + failing thresholds); reset/replay determinism probes pass; receipts committed in the radix evidence packet; `git diff --check` silent.
...
- **stop condition**: pause and route when — the FMIR seam cannot execute the exemplar proof (→ hand-2/Faber per §FMIR Gate); the oracle cannot load the 22.6 GB artifact or the probe dump is incomplete; a divergence cannot be attributed to a named layer/position (→ route to the owning surface, never a silent aggregate match); the proof would require a device handle in a Gradus value, unapproved paid infrastructure, or a main-branch write.
- **integrable**: yes (terminal unit).
- **est_work_tokens**: 6k–8k.
- **est_basis**: the executed-proof + receipts slice of the original 36k–72k — closest to the PML5-U6 aggregate oracle proof precedent (12k–24k) scaled down to the per-layer comparison of one prefill + one decode; plus the closeout docs; high end of the per-unit ranges.
- **tool_latency**: high — the exemplar's FMIR run is seconds, but each probe load of the 22.6 GB artifact (Metal) is minutes; the comparison + receipts are the executed proof.
...
**One aggregate gate, no child lane gates.** The GGUF-M3 completion is checked exactly once at the aggregate: **merge** integrates the landed M3 unit set (U1–U8) onto `factory/merge` and the GGUF-M3 done oracle — the executed per-layer agreement under the first-divergence rule (U8 receipt) with reset/replay deterministic — is accepted, with docs/status lines consistent. No M3 child unit carries `./scripta/check-source`, `./scripta/check-compile`, package `faber check`, `--stage`, `--e2e`, `--full`, or any broad suite; each child's `sanity` is one narrow proba of its own surface. **Per-unit rule-2 fields live in §5** (each unit carries its own first failing oracle, closeout command, expected observed result, est_basis, and stop condition — restored by the audit `2786c77d` P2 fix); the **aggregate fields stay lane-owned** and are named once here and in §8, never duplicated onto children.
