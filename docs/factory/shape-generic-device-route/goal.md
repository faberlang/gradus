...
**Status**: planned — forged from CTO research 0217b270 (2026-08-27); implementation dispatch gated on PGC waves + first post-PGC AC checkpoint; prerequisite unit SGR-U0 (imported generic entry instantiation — the hard gate) dispatched in parallel 2026-08-27 per operator order — LANDED 4245ce35e same day: route admission + fail-closed bindings (`imported_device_bindings_unresolved`); DUAL REVIEW per operator standing order — auditor clean_pass (`0f6d0faa`) + CTO proceed (`1dd6fa4d`); CTO dispositions carried to the lowering: SGD-2 binds the full identity tuple (source digest, ordered bindings, recipe, target/toolchain, emitted bytes), SGD-4 consumes a package-aware export API, evidence map re-verified pre-dispatch; AIR-census zero-rows ruled a separate CLI/reporting limitation
**Created**: 2026-08-27
