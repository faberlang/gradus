...
5. **Rope pair extent (U4b/c/d).** `P` (table rows `[T,P,3]`) is an independent size param because `d/2` is not assumed to be size arithmetic; `rope_norm(0, d)` replaces the `64` body literal. **CTO `44ab08c8` 2026-09-03: option (a)** — `IndexParam` is a legal value only at `NumerusIndex` intrinsic args (bare identifier, no arithmetic); AIR stays literal-only; substitution is pre-AIR. Reject arity-1 figura-derived dim. D5 amendment: one extra legal position, no new expression forms. Planner `60399b94` lowers the compiler Hands: `.vivi/planning/60399b94-u4b-cto-a-delivery.md` — **SGR-C1** (radix-semantic NumerusIndex-only admission; done_when = packet `faber check` accepts `rope_norm(0, d)`) → **SGR-C2** (radix-module pre-AIR substitution; AIR literal-only unchanged); U4b gains SGR-C2 as prerequisite. Do not re-freeze `64/32`.
