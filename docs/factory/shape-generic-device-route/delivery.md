...
| **U2** | Imported generic `@ kernel` instance is a device entry of the compiling package: `faber build -t metal-text` on `spike3-host-caller.fab` emits a kernel (sizes baked), not `metal-text requires at least one @ nucleum function` | `radix/crates/radix-module` (consume `imported_device_registrations` on the emit path; do not leave them empty on the package-relevant lower); `radix/crates/radix-program` if the join is still drop; `radix-mir-metal` only if entry selection ignores registrations | U1 | spike3 metal-text exit 0, emitted MSL has a kernel, sizes from the `[2,2]` instantiation; spike2 unchanged; fail-closed SGR-U0 issues still fire on missing bodies | `cargo test -p radix-module` package_instantiate + focused; `cargo run -p faber -- build -t metal-text` spike3 | No `kernel.fab` rewrite; no splice removal | yes |
