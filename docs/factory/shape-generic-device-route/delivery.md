...
5. **Rope pair extent (U4b/c/d).** `P` (table rows `[T,P,3]`) is an independent size param because `d/2` is not assumed to be size arithmetic; `rope_norm(0, d)` replaces the `64` body literal. If the compiler rejects param-count or inference here, the U4b Hand escalates through its handle — do not re-freeze `64/32`.
