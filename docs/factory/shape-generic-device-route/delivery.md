...
| **U2** | Metal-text emit path treats an imported generic `@ kernel` instance as an emit-able kernel. Diagnostic today: `radix-mir-metal/src/lib.rs` (~515) `"metal-text requires at least one @ nucleum function"` when `kernels` is empty. Package instantiate already fills `imported_device_registrations` (`radix-program/src/mir/lower.rs` ~2021). **Edit:** join that carrier into the metal-text kernel list (and/or stop requiring nucleum when Kernel-role instances exist). | `radix/crates/radix-mir-metal/src/lib.rs`; `radix-module` / `radix-program` only if registrations never reach that function | U1 | `faber build -t metal-text` on `gradus/scripta/spike-shape-generic-kernel/spike3-host-caller.fab` emits a kernel; sizes baked from `[2,2]`; spike2 still emits | that one metal-text command | No `kernel.fab`; no GEA3 splice; no PGC/SGD re-read | yes |
