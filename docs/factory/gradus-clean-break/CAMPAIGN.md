...
| 2 — author missing twins, delete remaining zoo | `mse<M,N>` twin then delete `mse_2x2/4x4/2x8`; per-channel `[N]` bias `linear` form and `layernorm<T,D>` twin then delete `linear_2x8`/`layernorm_2x8`; re-sweep `train_step_*` / `bert_tiny_block_2x8` / `forward_mlp_loss [4,4]` against whatever twins then exist | **Lowered** — need `e01c6cc0` + amendment `6732365e` (settled); units in [`wave-2-delivery.md`](wave-2-delivery.md) (lowering task `24f8fbca`), pending Mind tasking. Wave-1 gate satisfied: folded at gradus `bf55937` / examples `402ea2e`. Lane paper: `factory/krs-2` branch `c419b02` body-reroutes for `linear_2x8`/`layernorm_2x8`; `*_carrier` residuals shrink as callers pin (SEM014/SEM005 posture, not this campaign's rewrite) |
...
| 2 — author missing twins, delete remaining zoo | pending (lowered) | planner (lowered) / — | units in [`wave-2-delivery.md`](wave-2-delivery.md), all `pending` | need `e01c6cc0` + amendment `6732365e`; lowering task `24f8fbca`; 14 units, Mind tasking pending |
