...
only under `*_carrier` names (SEM014 load-edge). All thirteen remaining zoo
symbols are gone; docs describe the generic surface; the pml0 admit rows carry
...
The inventory below contains thirteen remaining symbols, verified against the
live declarations at the cited source lines; the five dense internal callers
are callers, not additional symbols.
...
| `done_when` | `grep -nE '^fn (layernorm|layernorm_carrier)(<|\()' src/nn.fab` shows exactly the typed `layernorm` and `layernorm_carrier` declarations (the anchored name boundary excludes `layernorm_2x8`); `grep -nE '^fn layernorm_2x8\(' src/nn.fab` still shows the preserved wrapper; `grep -rn "nn\.layernorm(" src/ exempla/` (open paren, excluding `layernorm_2x8`/`layernorm_carrier`) returns zero live calls; `faber check` green on gradus + `exempla/nn-bridge`; `layernorm_2x8` remains present and untouched |
| `depends_on` | none |
...
| `done_when` | `grep -nE '^fn (mse|mse_carrier)(<|\()' src/loss.fab` shows exactly the typed `mse` and `mse_carrier` declarations (the anchored name boundary excludes the preserved fixed-shape wrappers); `grep -nE '^fn mse_(2x2|4x4|2x8)\(' src/loss.fab` still shows the preserved wrappers; zero live `loss.mse(` staged calls outside `mse_carrier`/`mse_2x2`-family names (grep over `src/`, probas included); typed proba rows pin the three oracles; `faber check` green on gradus; `mse_2x2/4x4/2x8` untouched |
...
| `done_when` | `grep -rn "_sgd_family" src/optimize.fab src/train.fab src/optimize.proba src/train.proba` returns only explicitly marked historical provenance; the later U7-owned `exempla/training-loop-mlp` references are deferred to U7 and are outside this gate; `sgd_step<size Figura>` is `@ public` with the unchanged body; all four `train_step_*` signatures unchanged and their proba rows still pass; `faber check` green on gradus |
...
| `write_scope` | gradus only: `src/model/dense.fab`; `src/nn.fab` (branch (a) typed-gather implementation, or branch (b) append-only header record of the verbatim SEM016 diagnostic); `src/nn.proba` (branch (a) typed-gather rows only) |
...
| `done_when` | `grep -rn "train_step_2x2\|train_step_4x4\|train_step_bert" src/train.fab src/train.proba src/optimize.fab` returns only explicitly-marked retirement provenance; later U14-owned references in `src/tensor.fab` and `src/gradus.fab` are deferred to U14 and are outside this gate; the four rewritten proba rows pass; `faber check` green on gradus + `exempla/training-loop-mlp` |
...
| `done_when` | Cross-repo `rg -nU` census (`_2x2|_2x8|_4x4|train_step_|bert_tiny_block|forward_mlp_loss[[:space:]]*\([^)]*tensor<f32,[[:space:]]*\[[[:space:]]*4[[:space:]]*,[[:space:]]*4[[:space:]]*\][^)]*\)|dense[.]gather\b|dense[.]gather_step\b|gather_step\b` over `*.fab *.proba *.md *.toml`, both repos) returns zero **current public-surface claims** outside the enumerated exclusion set: (a) frozen evidence (oracle `reference.json`/receipts, benchmark baselines, `docs/archived/**`, `docs/deep-code-review-*`, landed `docs/factory/**` campaign paper except the pml0 marks this unit owns); (b) explicitly-marked retirement provenance in src ledger comments (U10-U13 done-whens); (c) nn-bridge baseline-red record. The `forward_mlp_loss` arm matches its full argument list and frozen `[4,4]` shape tokens across formatting whitespace; the qualified `dense.gather` and `dense.gather_step` arms cover both gather statue spellings. Generic call-site shapes and historical evidence remain allowed only under the stated classification. Every replacement names its destination (mse → `loss.mse<M,N>`, steps → `optimize.sgd_step<Figura>`, bert leaves → `nn.layernorm<T,D>`/`nn.linear_channel<M,K,N>`); api-reference lists every new typed symbol and no deleted one |
