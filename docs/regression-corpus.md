...
| Exempla consumers | `exempla/gradient-seam`, `exempla/gradient-seam-nolib`, `exempla/training-loop-mlp`, `exempla/token-generation`, `exempla/gguf-manifest`, `exempla/gguf-inspect`, `exempla/qwen36-35b-inference`, `exempla/dense-swiglu` | Public-surface consumers plus the executed GGUF synthetic proof (40 PASS / 0 FAIL), guarded six-file local inspection receipt, the capstone tokenizer-phase run (LIB-02-U4-1), and the executed REF-01-U1.2 SiLU/SwiGLU proof (14 PASS / 0 FAIL) |
...
| `src/nn.proba` | `gradus:nn` | GELU / layernorm / linear + **SiLU / SwiGLU (REF-01-U1.2)** — f64 pins @ **5e-4** |
