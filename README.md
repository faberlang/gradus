...
| `exempla/dense-prefill-smollm2` | REF-01-U1.9 SmolLM2 compiled-route prefill-logit consumer (U1.8 forward vs pinned llama.cpp golden at prompt-end) | **Stop** — `faber check` green; `faber build --target rust` CODEGEN001 (unresolved definition id in `tensor_view.fab`); no executed logits |
