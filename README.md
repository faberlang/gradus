...
| `exempla/dense-prefill-smollm2` | REF-01-U1.9 SmolLM2 compiled-route prefill-logit consumer (U1.8 forward vs pinned llama.cpp golden at prompt-end) | **Stop** — resume-2 at radix `3853d4b8f`: packet `faber` green; rust runtime plan `PKG001:host_provider_selection_invalid` (`processus:exi` absent from hosts manifest); no rust binary, no executed logits |
