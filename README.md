...
| `exempla/dense-prefill-qwen2` | REF-01-U1.10 Qwen2.5-0.5B real-file prefill consumer through `gradus:model/dense` `forward` | **Stop** — `faber check` green; resume cannot rebuild packet `faber` at radix `b919052f0` (`E0432` `ImportedEnumVariantInfo`); rust-target emit not reached; no executed logits |
