...
| `exempla/dense-prefill-smollm2` | REF-01-U1.9 SmolLM2 compiled-route prefill-logit consumer (U1.8 forward vs pinned llama.cpp golden at prompt-end) | **Stop** — resume at radix `b919052f0`: `cargo build -p faber` E0432 (`faber_hir_rust::ImportedEnumVariantInfo` unresolved); no new faber, no rust emit, no executed logits |
| `exempla/dense-prefill-qwen2` | REF-01-U1.10 Qwen2.5-0.5B real-file prefill consumer through `gradus:model/dense` `forward` | **Stop** — packet `faber` green at radix `3853d4b8f`; rust emit `PKG001:host_provider_selection_invalid` (`processus:exi` missing from hosts processus manifest); no executed logits |
